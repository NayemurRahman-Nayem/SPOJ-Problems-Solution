...
ll query(int node, int start, int end , int l,  int r , ll k ) {
    if (l > end || r < start) {
        return 0 ;
    }
    if (start >= l and end <= r) {
        return (upper_bound(mergeSortTree[node].v.begin(),mergeSortTree[node].v.end(),k)-mergeSortTree[node].v.begin()) ; 
    } 
    int mid = (start+end) / 2 ;
    int left = node*2 ;
    int right = node*2 + 1 ;
...
void solution(int cs) {
    ...
    int p = 1 ;
    map<ll,ll>compression , rv ; 
    for(auto it:st) {
        compression[it] = p ; 
        p++ ; 
    }
    for(int i=1;i<=n;i++) {
        rv[compression[a[i]]] = a[i] ; 
        a[i] = compression[a[i]] ; 
    }
    build(1,1,n) ; 
    for(int i=1;i<=m;i++) {          // m = 5000
        int l , r , k ; 
        cin >> l >> r >> k ; 
        ll low = 1 , high = n ; 
        int ans = -1 ; 
        while(low<=high) {                   // log(n) = 17 
            ll mid = (low + high) / 2 ;
            ll cnt = query(1,1,n,l,r,mid) ;    // log^2(n) = 17 * 17 
            if(cnt>=k) {
                ans = mid ; 
                high = mid - 1 ;  
            } 
